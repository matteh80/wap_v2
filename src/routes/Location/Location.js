...
class Location extends React.Component {
  ...
  highlightLocations () {
    let _self = this
    $('svg path').each(function (e, i) {
      let name = _self.getParentName($(this).attr('id'))
      let parent = _self.props.locations.locations.filter(locations => locations.name === name)
  ...
  getParentName (isoCode) {
    switch (isoCode) {
      case 'SE-AB': return 'Stockholms län'
      case 'SE-C': return 'Uppsala län'
      case 'SE-D': return 'Södermanlands län'
      case 'SE-E': return 'Östergötlands län'
      case 'SE-F': return 'Jönköpings län'
      case 'SE-G': return 'Kronobergs län'
      case 'SE-H': return 'Kalmar län'
      case 'SE-I': return 'Gotlands län'
      case 'SE-K': return 'Blekinge län'
      case 'SE-M': return 'Skåne län'
      case 'SE-N': return 'Hallands län'
      case 'SE-O': return 'Västra Götalands län'
      case 'SE-S': return 'Värmlands län'
      case 'SE-T': return 'Örebro län'
      case 'SE-U': return 'Västmanlands län'
      case 'SE-W': return 'Dalarnas län'
      case 'SE-X': return 'Gävleborgs län'
      case 'SE-Y': return 'Västernorrlands län'
  ...
  getSelectedInParent (parentName) {
    let parent = this.props.locations.locations.filter(locations => locations.name === parentName)
    let selected = parent[0].municipalities.filter(municipalities => municipalities.isChecked)
    return selected
  }

  getMunicipalities (parentName) {
    let parent = this.props.locations.locations.filter(locations => locations.name === parentName)
    let municipalities = parent[0].municipalities
    ...
    return (
      <Container fluid id='location'>
        ...
        <Row className='flex-row-reverse'>
          <SpeechBubble pos='side' hideable xs={12} md={12} xl={5}>
